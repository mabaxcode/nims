...
$( document ).ready(function() {
    validateAddGred('kt_modal_add_gred_form');
});

var validateFormAddGred;
function validateAddGred(formID)
{   
...
$(document).on('click', '.add-new-gred', function (e) {
    // e.preventDefault();
    ...
    validateFormAddGred.validate().then(function(status) {
        ...
        if (status == 'Valid') {
            ...
            var fileFormData = $('#kt_modal_add_gred_form').serialize();

            $.ajax({
                url: base_url + 'setting/addNewGred',
                type: "POST",
                data: fileFormData,
                dataType: "json",
                async: true,
